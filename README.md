...
### Issues
GameManager uses [`pydub`](https://github.com/jiaaro/pydub) library for audio playback. Unfortunately pydub's preferred backend `pyaudio` has [known issues](https://github.com/raspberrypi/linux/issues/994) running on a Raspberry Pi. I've also experienced these clipping issues and couldn't figure out a way around it. Getting `pydub` to work properly involves a mega-hack. Edit library file: [`/usr/local/lib/python2.7/dist-packages/pydub/playback.py`](https://github.com/jiaaro/pydub/blob/master/pydub/playback.py) and change this line:
